#pragma once

/*
* SDK generated by Dumper-8
*
* https://github.com/NotTacs/Dumper-8
*/

// Package: SpatialCustomizationSelection

#include "Basic.hpp"

#include "SpatialCustomizationSelection_classes.hpp"
#include "SpatialCustomizationSelection_parameters.hpp"


namespace SDK
{

// Function SpatialCustomizationSelection.SpatialCustomizationSelection_C.ExecuteUbergraph_SpatialCustomizationSelection
// (Final, UbergraphFunction, HasDefaults)
// Parameters:
// int32                                   EntryPoint                                             (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpatialCustomizationSelection_C::ExecuteUbergraph_SpatialCustomizationSelection(int32 EntryPoint)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpatialCustomizationSelection_C", "ExecuteUbergraph_SpatialCustomizationSelection");

	Params::SpatialCustomizationSelection_C_ExecuteUbergraph_SpatialCustomizationSelection Parms{};

	Parms.EntryPoint = EntryPoint;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpatialCustomizationSelection.SpatialCustomizationSelection_C.OnChoiceCountUpdated
// (Event, Protected, BlueprintEvent)
// Parameters:
// int32                                   ChoiceNum                                              (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// int32                                   TotalCount                                             (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpatialCustomizationSelection_C::OnChoiceCountUpdated(int32 ChoiceNum, int32 TotalCount)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpatialCustomizationSelection_C", "OnChoiceCountUpdated");

	Params::SpatialCustomizationSelection_C_OnChoiceCountUpdated Parms{};

	Parms.ChoiceNum = ChoiceNum;
	Parms.TotalCount = TotalCount;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpatialCustomizationSelection.SpatialCustomizationSelection_C.BndEvt__Button_PreviousChoice_K2Node_ComponentBoundEvent_1_CommonButtonClicked__DelegateSignature
// (BlueprintEvent)
// Parameters:
// class UCommonButton*                    Button                                                 (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, InstancedReference, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpatialCustomizationSelection_C::BndEvt__Button_PreviousChoice_K2Node_ComponentBoundEvent_1_CommonButtonClicked__DelegateSignature(class UCommonButton* Button)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpatialCustomizationSelection_C", "BndEvt__Button_PreviousChoice_K2Node_ComponentBoundEvent_1_CommonButtonClicked__DelegateSignature");

	Params::SpatialCustomizationSelection_C_BndEvt__Button_PreviousChoice_K2Node_ComponentBoundEvent_1_CommonButtonClicked__DelegateSignature Parms{};

	Parms.Button = Button;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpatialCustomizationSelection.SpatialCustomizationSelection_C.BndEvt__Button_NextChoice_K2Node_ComponentBoundEvent_0_CommonButtonClicked__DelegateSignature
// (BlueprintEvent)
// Parameters:
// class UCommonButton*                    Button                                                 (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, InstancedReference, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpatialCustomizationSelection_C::BndEvt__Button_NextChoice_K2Node_ComponentBoundEvent_0_CommonButtonClicked__DelegateSignature(class UCommonButton* Button)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpatialCustomizationSelection_C", "BndEvt__Button_NextChoice_K2Node_ComponentBoundEvent_0_CommonButtonClicked__DelegateSignature");

	Params::SpatialCustomizationSelection_C_BndEvt__Button_NextChoice_K2Node_ComponentBoundEvent_0_CommonButtonClicked__DelegateSignature Parms{};

	Parms.Button = Button;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpatialCustomizationSelection.SpatialCustomizationSelection_C.OnFinalizeChoiceFinished
// (Event, Protected, BlueprintEvent)
// Parameters:
// bool                                    bSucceeded                                             (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor)

void USpatialCustomizationSelection_C::OnFinalizeChoiceFinished(bool bSucceeded)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpatialCustomizationSelection_C", "OnFinalizeChoiceFinished");

	Params::SpatialCustomizationSelection_C_OnFinalizeChoiceFinished Parms{};

	Parms.bSucceeded = bSucceeded;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpatialCustomizationSelection.SpatialCustomizationSelection_C.OnIntroChoiceCompleted
// (BlueprintCallable, BlueprintEvent)

void USpatialCustomizationSelection_C::OnIntroChoiceCompleted()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpatialCustomizationSelection_C", "OnIntroChoiceCompleted");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpatialCustomizationSelection.SpatialCustomizationSelection_C.Construct
// (BlueprintCosmetic, Event, Public, BlueprintEvent)

void USpatialCustomizationSelection_C::Construct()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpatialCustomizationSelection_C", "Construct");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpatialCustomizationSelection.SpatialCustomizationSelection_C.BP_OnActivated
// (Event, Protected, BlueprintEvent)

void USpatialCustomizationSelection_C::BP_OnActivated()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpatialCustomizationSelection_C", "BP_OnActivated");

	UObject::ProcessEvent(Func, nullptr);
}


// Function SpatialCustomizationSelection.SpatialCustomizationSelection_C.OnUpdateFinalizedText
// (Event, Protected, HasOutParams, BlueprintEvent)
// Parameters:
// const class FText&                      InFinalizedChoiceText                                  (ConstParm, BlueprintVisible, BlueprintReadOnly, Parm, OutParm, ReferenceParm)

void USpatialCustomizationSelection_C::OnUpdateFinalizedText(const class FText& InFinalizedChoiceText)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpatialCustomizationSelection_C", "OnUpdateFinalizedText");

	Params::SpatialCustomizationSelection_C_OnUpdateFinalizedText Parms{};

	Parms.InFinalizedChoiceText = std::move(InFinalizedChoiceText);

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpatialCustomizationSelection.SpatialCustomizationSelection_C.OnCategoryUpdated
// (Event, Protected, BlueprintEvent)
// Parameters:
// ESpatialCustomizationCategoryState      CategoryState                                          (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// const struct FLinearColor&              Color                                                  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// bool                                    bUseColorPicker                                        (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor)

void USpatialCustomizationSelection_C::OnCategoryUpdated(ESpatialCustomizationCategoryState CategoryState, const struct FLinearColor& Color, bool bUseColorPicker)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpatialCustomizationSelection_C", "OnCategoryUpdated");

	Params::SpatialCustomizationSelection_C_OnCategoryUpdated Parms{};

	Parms.CategoryState = CategoryState;
	Parms.Color = std::move(Color);
	Parms.bUseColorPicker = bUseColorPicker;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpatialCustomizationSelection.SpatialCustomizationSelection_C.HandleCategoryUpdated
// (Public, HasDefaults, BlueprintCallable, BlueprintEvent)
// Parameters:
// ESpatialCustomizationCategoryState      State                                                  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// const struct FLinearColor&              Color                                                  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// bool                                    UseColorPicker                                         (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor)

void USpatialCustomizationSelection_C::HandleCategoryUpdated(ESpatialCustomizationCategoryState State, const struct FLinearColor& Color, bool UseColorPicker)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpatialCustomizationSelection_C", "HandleCategoryUpdated");

	Params::SpatialCustomizationSelection_C_HandleCategoryUpdated Parms{};

	Parms.State = State;
	Parms.Color = std::move(Color);
	Parms.UseColorPicker = UseColorPicker;

	UObject::ProcessEvent(Func, &Parms);
}


// Function SpatialCustomizationSelection.SpatialCustomizationSelection_C.HandleChoiceCountUpdated
// (Public, HasDefaults, BlueprintCallable, BlueprintEvent)
// Parameters:
// int32                                   ChoiceNum                                              (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// int32                                   TotalCount                                             (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void USpatialCustomizationSelection_C::HandleChoiceCountUpdated(int32 ChoiceNum, int32 TotalCount)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("SpatialCustomizationSelection_C", "HandleChoiceCountUpdated");

	Params::SpatialCustomizationSelection_C_HandleChoiceCountUpdated Parms{};

	Parms.ChoiceNum = ChoiceNum;
	Parms.TotalCount = TotalCount;

	UObject::ProcessEvent(Func, &Parms);
}

}

